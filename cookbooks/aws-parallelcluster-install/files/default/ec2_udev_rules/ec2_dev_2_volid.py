...
def validate_device_name(device_name):
    ...
    """
    match = re.match(r"^(\w){2,}$", device_name)
    if not match:
        raise ValueError("Device name provided argument contains has an invalid pattern.")
    return True
...
def main():
    ...
    try:
        dev = str(sys.argv[1])
        print(f"Starting ec2_dev_2_volid.py script with dev: {dev}")
