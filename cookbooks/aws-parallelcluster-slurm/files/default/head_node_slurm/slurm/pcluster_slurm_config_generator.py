...
def generate_slurm_config_files(
    ...
    # Assumes one queue and one compute resource from here on
    nodes = queues[0]["ComputeResources"][0]["MaxCount"]
...
def _get_head_node_config():
    return {
        "head_node_hostname": "ip-10-0-0-134", #gethostname(),
...
def _generate_queue_config(
    ...
    for node_index in range(1, nodes+1):
        ...
        if not dryrun:
            filename = path.join(output_dir, f"slurm_parallelcluster_{queue_name}_{node_index}_{file_type}.conf")
